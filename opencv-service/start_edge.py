...
def register_camera_url(camera_url: str):
    ...
    try:
        r = requests.post(
            f"{CENTRAL_API_URL}/api/camera/update",
            json={
                "lotId":     LOT_ID,
        ...
        if r.status_code == 200:
            ...
            print(f"[Registry] ✗ {r.status_code}: {r.text}", flush=True)
...
def main():
    ...
    try:
        r = requests.get(f"{CENTRAL_API_URL}/api/health", timeout=5)
        if r.status_code == 200:
            print("[Step 1] ✓ Central API is up", flush=True)
        else:
            print(f"[Step 1] ✗ Central API returned {r.status_code}", flush=True)
    except Exception as e:
        print(f"[Step 1] ✗ Central API unreachable: {e}", flush=True)
    ...
    if camera_url:
        try:
            r = requests.get(camera_url, timeout=3, stream=True)
            if r.status_code == 200:
                print(f"[Step 2] ✓ Camera reachable: {camera_url}", flush=True)
                camera_ok = True
                r.close()
        except Exception:
            print(f"[Step 2] ✗ Camera at {camera_url} is unreachable", flush=True)
    ...
    if not camera_ok:
        ...
        if discovered:
            ...
            print(f"[Step 3] ✓ Discovered: {camera_url}", flush=True)
            # Update .env.local
            ...
            try:
                ...
                if "CAMERA_IP=" in content:
                    lines = content.split('\n')
                    lines = [
                        f"CAMERA_IP={ip_port}" if l.startswith("CAMERA_IP=") else l
                        for l in lines
                    ]
                    content = '\n'.join(lines)
                else:
                ...
                with open(env_path, 'w') as f:
                    f.write(content)
                print(f"[Step 3] ✓ Updated .env.local with CAMERA_IP={ip_port}", flush=True)
            except Exception as e:
                print(f"[Step 3] Could not update .env.local: {e}", flush=True)
        else:
            print("[Step 3] ✗ No cameras found on local network", flush=True)
            print("[Step 3]   Please ensure IP Webcam app is running on your phone", flush=True)
